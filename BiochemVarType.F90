...
!=== define "state" sub-type of biochem (biochem%state%variable)
  type :: state_type
    ...
    real(kind=kind_noahmp), allocatable, dimension(:) :: PerRMass      ! root biomass per layer [g/m2] MENG202406
...
!=== define "parameter" sub-type of biochem (biochem%param%variable)
  type :: parameter_type
    ...
    real(kind=kind_noahmp) :: RootGrowDirection          ! crop root grow direction parameter edited by meng 202405
    real(kind=kind_noahmp) :: RootDistribution           ! crop root distribution parameter edited by meng 202405
    real(kind=kind_noahmp) :: MaximumRootDepth           ! Maxmium root depth [m] MENG202406
    real(kind=kind_noahmp) :: OptRootTemperature         ! optimum temperature for root growth [C] MENG202406
    real(kind=kind_noahmp) :: MinRootTemperature         ! minimum temperature for  roots growth [C] MENG202406
    real(kind=kind_noahmp) :: LethalRootTemperature      ! lethal temperature for root growth [C] MENG202406
    real(kind=kind_noahmp) :: SpecificRootLength         ! the length of the root system per unit weight [C] MENG202406 
