...
! -------------------------------------------------
    ...
    if ( .not. allocated (NoahmpIO%RDXY)      ) allocate ( NoahmpIO%RDXY       (XSTART:XEND,               YSTART:YEND) ) ! root depth [m] edited by meng202405
    if ( .not. allocated (NoahmpIO%JMXY)      ) allocate ( NoahmpIO%JMXY       (XSTART:XEND,               YSTART:YEND) ) ! depth of maximum root density [cm] MENG202406
    if ( .not. allocated (NoahmpIO%RFXY)      ) allocate ( NoahmpIO%RFXY       (XSTART:XEND,1:NSOIL,       YSTART:YEND) ) ! root fraction in each soil layer (0-1)MENG
    if ( .not. allocated (NoahmpIO%RLDXY)     ) allocate ( NoahmpIO%RLDXY      (XSTART:XEND,1:NSOIL,       YSTART:YEND) ) ! root length density [m/m3] MENG202406
    if ( .not. allocated (NoahmpIO%PRMASSXY)  ) allocate ( NoahmpIO%PRMASSXY   (XSTART:XEND,1:NSOIL,       YSTART:YEND) ) ! root biomass per layer [g/m2] MENG202406
    ...
    if ( .not. allocated (NoahmpIO%DBXY)      ) allocate ( NoahmpIO%DBXY       (XSTART:XEND,               YSTART:YEND) ) ! thickness of canopy buried by snow (m)
    ...
    NoahmpIO%RDXY            = undefined_real  ! edited by meng 202405
    NoahmpIO%RFXY            = undefined_real  ! edited by meng 202405
    NoahmpIO%RLDXY           = undefined_real  ! MENG202406
    NoahmpIO%PRMASSXY        = undefined_real  ! MENG202406
    NoahmpIO%JMXY            = undefined_int  ! MENG202406
