...
!=== initialize with default values
  subroutine EnergyVarInitDefault(noahmp)
    ...
    noahmp%energy%state%RootDepth                   = undefined_real ! edited by meng 202405
    ...
    noahmp%energy%state%DepthMaxRootDensity         = undefined_int  !MENG202406
    ...
    if ( .not. allocated(noahmp%energy%state%RootFraction) )        &
       allocate( noahmp%energy%state%RootFraction(1:NumSoilLayer) )
    if ( .not. allocated(noahmp%energy%state%RootLengthDensity) )   &
       allocate( noahmp%energy%state%RootLengthDensity(1:NumSoilLayer) )
   ! edited by meng 202405
    noahmp%energy%state%RootLengthDensity   (:)     = undefined_real ! MENG202406
    noahmp%energy%state%RootFraction        (:)     = undefined_real ! edited by meng 202405
    ...
    if ( .not. allocated(noahmp%energy%param%ClayContent) )        &
       allocate( noahmp%energy%param%ClayContent(1:NumSoilLayer) )
    if ( .not. allocated(noahmp%energy%param%SandContent) )        &
       allocate( noahmp%energy%param%SandContent(1:NumSoilLayer) )
    if ( .not. allocated(noahmp%energy%param%SoilBulkDensity) )    &
       allocate( noahmp%energy%param%SoilBulkDensity(1:NumSoilLayer) ) 
! MENG202406
    ...
    noahmp%energy%param%ClayContent       (:)       = undefined_real !MENG202406
    noahmp%energy%param%SandContent       (:)       = undefined_real !MENG202406
    noahmp%energy%param%SoilBulkDensity   (:)       = undefined_real !MENG202406
