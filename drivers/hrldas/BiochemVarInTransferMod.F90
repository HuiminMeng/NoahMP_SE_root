...
! -------------------------------------------------------------------------
    ...
    noahmp%biochem%state%PerRMass(1:NumSoilLayer)   = NoahmpIO%PRMASSXY(I,1:NumSoilLayer,J)  ! MENG202406
    ...
    if ( (OptCropModel > 0) .and. (CropType > 0) ) then
       ...
       noahmp%biochem%param%RootGrowDirection       = NoahmpIO%RGD_TABLE      (CropType) ! edited by meng 202405
       noahmp%biochem%param%RootDistribution        = NoahmpIO%RDI_TABLE      (CropType) ! edited by meng 202405
       noahmp%biochem%param%MaximumRootDepth        = NoahmpIO%RDMAX_TABLE    (CropType) ! MENG202406
       noahmp%biochem%param%OptRootTemperature      = NoahmpIO%RTOPT_TABLE    (CropType) ! edited by meng 202406
       noahmp%biochem%param%MinRootTemperature      = NoahmpIO%RTB_TABLE      (CropType) ! edited by meng 202406
       noahmp%biochem%param%LethalRootTemperature   = NoahmpIO%RTMIN_TABLE    (CropType) ! edited by meng 202406
       noahmp%biochem%param%SpecificRootLength      = NoahmpIO%RORT_TABLE     (CropType) ! edited by meng 202406
